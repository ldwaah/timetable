...
INTRO_REPLACEMENTS = {
    "facilities": (
        "Christa recognises that some elements of her current role overlap with "
        "the proposed role, particularly administrative and coordination tasks. "
        "Her concern is not that there is no overlap, but that the proposed "
    ...
    "consultation": (
        ...
        "notes set out Christa's position on timing, what remained open, and "
...
def add_title_page(doc: Document, title: str, subtitle: str | None = None) -> None:
    ...
    r3 = p3.add_run("Prepared for: Christa Dawson")
...
def _clean_prefix(line: str) -> str:
    ...
    if m:
        ...
        if low.startswith("support "):
            return f"In the proposed role, Christa would {role[0].lower()}{role[1:]}"
...
def build_comparative_analysis() -> Document:
    ...
    add_footer(doc, "Prepared for: Christa Dawson")
    return doc
